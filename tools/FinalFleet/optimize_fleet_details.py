...
import bpy,bmesh,ast,struct,gzip,io,json,shutil
...
backup=R/'.build/fleet-detail-before';backup.mkdir(exist_ok=True)
destination=R/'design/fleet-detail-optimization';destination.mkdir(exist_ok=True)
summary=[]
for path in sorted((R/'assets/ships/final').glob('*.bin.gz')):
 if path.name=='MercantShip.bin.gz':continue # Óttarr has already received this pass.
 original=backup/path.name
 if not original.exists():shutil.copy2(path,original)
 magic,spec,header,parts,tail=unpack(original)
 new=[];report=[]
 for tag,mi,vs,ts in parts:
  if tag in ['hull','sail','rudder','mast','paddle']:
 ...
 stream.write(tail)
 out=destination/path.name.removesuffix('.bin.gz');out.mkdir(exist_ok=True)
 (out/path.name).write_bytes(gzip.compress(stream.getvalue(),mtime=0))
 report=dict(before=sum(len(t)//3 for _,_,_,t in parts),after=sum(len(t)//3 for _,_,_,t in new),parts=report,protected=['hull','sail','rudder','mast','paddle'])
 (out/'optimization.json').write_text(json.dumps(report,indent=2)+'\n');summary.append(dict(asset=path.name,**report));print(path.name,report['before'],report['after'],flush=True)
(destination/'optimization.json').write_text(json.dumps(summary,indent=2)+'\n')
