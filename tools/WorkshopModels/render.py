...
R=Path(__file__).resolve().parents[2];OUT=R/'design/workshop-supplies-v1'
...
for item in models:
 ...
 cam.location=center+Vector((4,-6,3.8 if key in ['workbench','tool-rack','caulking-station','rigging-rack','paint-stand'] else 6))*factor
 ...
 scene.render.resolution_x=1400 if key in ['workbench','slipway'] else 760;scene.render.resolution_y=1050 if key in ['workbench','slipway'] else 680;scene.render.resolution_percentage=100;scene.cycles.samples=32;scene.render.threads=6
