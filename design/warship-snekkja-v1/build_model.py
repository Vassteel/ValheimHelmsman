...
for i,x in enumerate(row_x):
 ...
 for y in [-w*.35,w*.35]:
  ...
  attachments.append({'id':'row_%02d_%s'%(i,'port' if y>0 else 'starboard'),'kind':'rowing seat guide','position':[x,y,z+.05],'facing':[-1,0,0]})
# Thirteen pairs of continuous carved rowing oars, posed through the actual ports.
oar_objects=[]
for side in [-1,1]:
 for i,x in enumerate(row_x):
  hx=x+.20;hp=Vector(hull(hx,.93,side));hy=abs(hp.y)
  # y distance outward from centre and height fall along the shaft.
  sections=[(.13,.028),(.32,.038),(hy,.042),(hy+1.46,.043),(hy+1.78,.068),(hy+2.1,.105),(hy+2.72,.105),(hy+2.88,.064)]
  verts=[];faces=[];N=8
  for y,ww in sections:
   z=hp.z-(y-hy)*.27
   for k in range(N):verts.append((hx+ww*cos(k*2*pi/N),side*y,z+(.035 if y<hy+1.75 else .019)*sin(k*2*pi/N)))
  ...
  ob=mesh('Rowing oar %02d %s'%(i,'port' if side==1 else 'starboard'),verts,faces,spar);oar_objects.append(ob)
...
report={'name':'WarShip snekkja study','standalone':True,'reference':'Skuldelev 5 and Helge Ask, Viking Ship Museum','archaeological_target_length_m':17.3,'archaeological_target_beam_m':2.47,'archaeological_target_depth_midships_m':1.16,'rowing_pairs':13,'oars':26,'shields_displayed':26,'projected_sail_area_m2':45.9155,'textures':'Original 128px maps; embedded','interpretation':'Original hull lines, rig, equipment placement and colour arrangement; not an exact archaeological reconstruction','limitations':['No game integration or player movement tests','Rowing oars shown in static review pose','Exact paint scheme is not established for the excavated ship']}
