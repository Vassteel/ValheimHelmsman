...
Each variant in styles/ includes an editable Blender scene, a ship-only GLB and an actual model render. styles/styles.json records the colours and file mappings. build_styles.py reproduces the variants from ottar.blend using bpy 5.1. The original plain model remains available at the top level.
