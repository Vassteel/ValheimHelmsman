...
for side in [-1,1]:
 for j in range(11):
  ...
  mesh(('Port' if side==1 else 'Starboard')+' clinker strake %02d'%(j+1),verts,faces,paint if j==10 else pine[(j+(side==1))%7])
...
# Additional soft cargo fills the visible voids around the barrel load and mast.
extra_before=set(ship.objects)
for i,x in enumerate([-3.45,-2.75,-2.05,-1.35,-.65,.65,1.35,2.05,2.75,3.45]):
 sack(x,-1.39,1.44,r=.23,h=.56+(i%3)*.05,label='Cargo / gap-packed sack')
for x in [-3.40,-2.60,-1.80,-1.0,1.0,1.80,2.60,3.40]:
 sack(x,.40,1.72,r=.22,h=.43,label='Cargo / gap-packed sack')
for x in [-.52,.52]:
 hide_bale(x,-.47,1.78,w=.61,d=.65,h=.50)
 sack(x,.48,1.66,r=.28,h=.61,label='Cargo / gap-packed sack')
for x in [-2.95,-1.95,1.95,2.95]:
 bundle(x,0,2.07,length=.66,r=.21,label='Cargo / topping roll')
# A compact load directly around the mast foot closes the remaining central void.
for y in [-.70,.65]:sack(0,y,1.72,r=.27,h=.56,label='Cargo / mast-foot sack')
for ob in set(ship.objects)-extra_before:
...
sack(5.55,0,deck_height(5.55),r=.27,h=.55,label='Cargo / provisions sack')
sack(6.23,.58,deck_height(6.23),r=.22,h=.45,label='Cargo / provisions sack')
barrel(5.60,1.26,deck_height(5.60),r=.19,h=.44)
hide_bale(-5.0,.38,deck_height(-5.0),w=.62,d=.53,h=.34)
sack(-6.27,.73,deck_height(-6.27),r=.23,h=.47,label='Cargo / stern provisions sack')
bundle(-4.92,-1.42,deck_height(-4.92)+.14,length=.68,r=.13,label='Cargo / rolled fishing net')
...
report={'name':'Ottar','status':'Standalone visual study. Not imported, registered, packaged or installed in Helmsman.',
 ...
 'units':'metres; Blender X forward, Y port, Z up','nominal_hull_length':15.84,'nominal_beam':4.8,'revision':6,'cargo_barrels':14,'cargo_hide_bales':14,'cargo_sacks':18,'cargo_rolled_bundles':10,'cargo_timber_bundles':2,'cargo_gap_sacks':22,'grating_panels':0,'helm_benches':1,'passenger_seats':4,'mast_holdfasts':1,'boarding_ladders':2,
 ...
 'limitations':['Not an exact archaeological reconstruction','Walking and attachment guides prepared; no Unity collision, seated animations or functional interactions','Original 128px colour textures embedded in GLB and Blender; subtle bump in Blender only']}
...
# Close inspection of the continuous carved steering oar and its bearing.
camera_at((-11,-16,9),(-5.7,-1.15,1.40),7.2)
scene.render.filepath=str(ROOT/'ottar-steering.png');bpy.ops.render.render(write_still=True)
print('Steering oar inspection saved',flush=True)
