...
assert not any(o.name.startswith('Rowing oar ') for o in bpy.data.objects)
...
assert not any(a.get('name','').startswith('Rowing oar ') for a in doc['nodes'])
assert not doc.get('cameras')
assert not any('Studio' in a.get('name','') for a in doc['nodes'])
print('PASS: 13 thwarts, zero rowing oars, 26 shields; closed steering oar clear of hull; finite geometry; embedded textures and ship-only GLB.')
print('Historically informed visual study only; no in-game movement or flotation acceptance.')
