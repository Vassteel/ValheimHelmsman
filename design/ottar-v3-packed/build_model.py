...
for x,y in [(-2.4,-.48),(2.42,-.46)]:sack(x,y,1.50,r=.22,h=.47)

# Long rolled hide/cloth bundles, rope-bound and strapped over the deck rails.
...
for side in [-1,1]:
 x=2.0;y=side*2.27;z=2.56
 bundle(x,y,z,length=1.62,r=.26,label='Cargo / rail-lashed hide roll')
 for dx in [-.5,.5]:
  tube('Cargo / rail securing lashing',[(x+dx,side*1.98,2.12),(x+dx,side*2.47,2.28),(x+dx,side*2.48,2.74),(x+dx,side*2.09,2.78),(x+dx,side*1.98,2.12)],.029,rope,6)
 # Spare oars have their own ties to the aft rails.
 for xx in [-2.6,.35]:
  tube('Cargo / oar rail lashing',[(xx,side*2.13,2.05),(xx,side*2.59,2.21),(xx,side*2.60,2.43),(xx,side*2.17,2.45),(xx,side*2.13,2.05)],.025,rope,6)
...
def seat(name,x,y,width,helm=False):
 ...
 if not helm:
  for dy in [-width*.40,width*.40]:box(name+' / back post',(x-.22,y+dy,top+.18),(.065,.065,.43),oak,.012)
  box(name+' / back rail',(x-.22,y,top+.32),(.09,width,.15),deckm[2],.016)
...
walking={'status':'Guide only; walking on visual cargo requires game integration and foot-contact testing',
 ...
 'review_note':'Use one smooth support rather than individual barrel/bag colliders. Review visible feet against uneven cargo and keep helm, passenger-seat approaches and mast holdfast accessible.'}
...
report={'name':'Ottar','status':'Standalone visual study. Not imported, registered, packaged or installed in Helmsman.',
 ...
 'units':'metres; Blender X forward, Y port, Z up','nominal_hull_length':15.84,'nominal_beam':4.8,'revision':3,'cargo_barrels':12,'cargo_hide_bales':6,'cargo_sacks':10,'cargo_rolled_bundles':4,'cargo_timber_bundles':2,'grating_panels':0,'helm_benches':1,'passenger_seats':4,'mast_holdfasts':1,
