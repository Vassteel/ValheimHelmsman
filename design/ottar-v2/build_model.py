...
for x in [-2.8,-1.95,-1.10,1.10,1.95,2.8]:
 for y in [-.62,.24]:barrel(x,y,.72)
for x,y,z in [(-2.7,1.0,.72),(-1.8,1.0,.72),(1.6,1.0,.72),(2.5,1.0,.72)]:
...
# Bundled trade timber rests on top of the wrapped bales, below the grating.
for x in [-2.25,2.05]:
 for y in [.80,1.01,1.22]:
  box('Cargo / bundled timber',(x,y,1.44),(1.50,.18,.16),deckm[1],.016)
 for dx in [-.48,.48]:
  tube('Cargo / timber lashing',[(x+dx,.70,1.37),(x+dx,.70,1.54),(x+dx,1.32,1.54),(x+dx,1.32,1.37),(x+dx,.70,1.37)],.020,rope,6)

# Four lift-out timber grate panels cover the hold with a level working deck.
# Their lattice is visual; a smooth walking collider will span its small holes.
grate_objects=[]
def grate_box(name,c,d,m):
 ob=box('Cargo grating / '+name,c,d,m,.008);ob['future_toggle']='Cargo grating';grate_objects.append(ob);return ob
for end in [-1,1]:
 for side in [-1,1]:
  cx=end*2.015;cy=side*.89;length=3.46;width=1.58
  # Supporting frame and lattice upper faces all finish at 1.84 m.
  for dx in [-length/2,length/2]:grate_box('end frame',(cx+dx,cy,1.79),(.10,width+.10,.10),oak)
  for dy in [-width/2,width/2]:grate_box('side frame',(cx,cy+dy,1.79),(length-.10,.10,.10),oak)
  for i in range(7):
   y=cy-width/2+.18+i*(width-.36)/6
   grate_box('longitudinal lath',(cx,y,1.81),(length-.10,.065,.06),deckm[1])
  for i in range(14):
   x=cx-length/2+.20+i*(length-.40)/13
   grate_box('cross lath',(x,cy,1.754),(.068,width-.10,.052),deckm[2])
# A broad central cross-plank and seam strips close gaps at the mast and bay edges.
grate_box('mast cross-plank',(0,0,1.79),(.47,3.46,.10),deckm[1])
for end in [-1,1]:grate_box('centre seam',(end*2.015,0,1.79),(3.56,.10,.10),oak)
for x in [-3.87,3.87]:grate_box('deck threshold',(x,0,1.79),(.15,3.46,.10),deckm[2])
for ob in ship.objects:
 if ob.name.startswith('Cargo /'):
  ob['appearance_only']=True;ob['future_collision']='none'
# Explicit engineering handoff, not a claim that Unity collision is implemented.
walking={'status':'Prepared guide only; requires Unity integration and movement testing',
 'coordinates':'Blender local metres: X bow, Y port, Z up',
 'cargo_collision':'none',
 'hold_walk_surface':{'type':'BoxCollider','center':[0,0,1.79],'size':[7.90,3.50,.10],'top':1.84},
 'export_axes':'GLB local axes: X bow, Y up, Z starboard. Before Valheim integration align the bow to ship forward (+Z); then the walking box is centred at [0,1.79,0] with size [3.50,.10,7.90]. Verify the importer transform.',
 'checks':['Walk continuously across fore deck, grated hold and aft deck','Verify collision stays with the moving ship','Keep mast and gunwale collisions separate','Verify swimming threshold and water masking inside loaded hull','Do not add individual cargo or lattice-hole colliders']}
(ROOT/'walking-surface.json').write_text(json.dumps(walking,indent=2)+'\n')
guides=bpy.data.collections.new('COLLISION GUIDES - not included in GLB');scene.collection.children.link(guides)
ob=box('GUIDE / smooth hold walking surface',(0,0,1.79),(7.90,3.50,.10),oak,0)
move(ob,guides);ob.display_type='WIRE';ob.hide_render=True;ob.hide_set(True)
ob['unity_component']='BoxCollider';ob['guide_only']=True
...
report={'name':'Ottar','status':'Standalone visual study. Not imported, registered, packaged or installed in Helmsman.',
 ...
 'units':'metres; Blender X forward, Y port, Z up','nominal_hull_length':15.84,'nominal_beam':4.8,'revision':2,'cargo_barrels':12,'cargo_bales':4,'cargo_timber_bundles':2,'grating_panels':4,
 ...
 'limitations':['Not an exact archaeological reconstruction','Walking collider guide prepared; no Unity colliders, LODs, sail animation or network components','GLB uses base material colours; Blender has procedural grain bump']}
