...
## Revision 2 - cargo grating

Four lift-out wooden lattice panels cover a fuller cargo load: twelve barrels, four wrapped bales and two bound timber bundles. Their top surface is 1.84 m above the model origin, nearly level with the edges of the existing end decks. This grating layout is a gameplay design choice, not a documented feature of the historical vessel.

Cargo is tagged for no collision. walking-surface.json and the hidden COLLISION GUIDES collection define one smooth hold-walking box for later integration. It spans the lattice holes so feet should not snag. The exported GLB contains only visible ship geometry; the collider is a specification, not active game physics. Mast and gunwale collisions, waterline behaviour, boarding and walking aboard a moving ship still require game implementation and testing.
