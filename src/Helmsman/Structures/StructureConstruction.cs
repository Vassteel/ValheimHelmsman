...
public sealed class StructureConstruction:MonoBehaviour
{
    ...
    internal string Validate(Blueprint plan,Vector3 origin,Quaternion rotation,Player player)
    {
        ...
        if(!StructureImports.Allowed||!yard||!yard.Near(player))return "Start construction while beside the puffin bench as an administrator.";
        ...
        {
            var prefab=ZNetScene.instance.GetPrefab(p.Prefab);var piece=prefab?prefab.GetComponent<Piece>():null;
            if(!piece||prefab!.GetComponent<Ship>()||prefab.GetComponent<StructureConstruction>())return p.Prefab+" is not a supported building piece.";
            if(Recipe(piece).Length==0)return p.Prefab+" has no construction recipe.";
    ...
    internal static string Recipe(Piece piece)=>string.Join(",",piece.m_resources.Where(r=>r.m_resItem&&r.m_amount>0).GroupBy(r=>Utils.GetPrefabName(r.m_resItem.gameObject)).OrderBy(g=>g.Key,StringComparer.Ordinal).Select(g=>g.Key+":"+g.Sum(r=>r.m_amount)));
    internal string Status
    {
        get
        {
            ...
            string need=s.recipe.Length>0?WorkshopMaterialSupply.Missing(s.recipe,s.supplied):"Checking the next foundation piece";
            return s.name+" · "+s.next+" / "+s.pieces.Length+" pieces\n"+s.status+(need.Length>0?"\nNeeded next: "+need:"");
        }
    }
    ...
    internal Dictionary<string,int> RemainingCosts()
    {
        ...
        {
            var prefab=ZNetScene.instance.GetPrefab(s.pieces[i].Prefab);if(!prefab)continue;
    ...
    private void Advance(StructureOrder s)
    {
        ...
        WorkshopMaterialSupply.Collect(player,yard,recipe,s.supplied,s.creator,r=>{s.supplied=r;Save(s);});
        if(!ConstructionFunding.Complete(recipe,s.supplied)){s.status="Waiting for Quartermaster or bench supplies";Save(s);return;}
