...
void Reset(){Plugin.Instance=new();Plugin.Solo=true;player=Player.m_localPlayer=new();player.Inventory.Items.Add(item);InventoryGui.instance=new();}
...
foreach(var cause in new[]{"remote","nonplayer","dead","missing","container","multiplayer","attack","dodge"})
{
 Reset();Humanoid actor=player;Inventory inventory=null;
 switch(cause){case "remote":actor=new Player();break;case "nonplayer":actor=new Humanoid();break;case "dead":player.Dead=true;break;case "missing":player.Inventory.Items.Clear();break;case "container":inventory=new Inventory();inventory.Items.Add(item);break;case "multiplayer":Plugin.Solo=false;break;case "attack":player.Attacking=true;break;case "dodge":player.Dodging=true;break;}
