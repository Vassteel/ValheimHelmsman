...
with tempfile.TemporaryDirectory(prefix='helmsman-materials-') as tmp:
 ...
 (p/'Resolver.cs').write_text('using System;using System.Linq;using System.Collections.Generic;using UnityEngine;using Jotunn.Managers;namespace Helmsman;internal static class ImportedShipMaterials {internal static Material wood,cloth;private static Dictionary<string,Shader> native=new();'+resolver+'}')
 ...
 Reset();var hull=new Material("longship hull","Custom/Static");var sail=new Material("Cloth sail","Custom/Cloth");var bench=new Material("wood","Custom/Piece");
 Add("VikingShip",hull,sail);Add("piece_workbench",bench);ImportedShipMaterials.Prepare();
 Check(ImportedShipMaterials.wood==bench,"Hull without Piece shader still registers using native workbench wood");
